...
public abstract class Weapon : ScriptableObject, IDamaging {
	...
	public float BulletRenderRange { get => bulletRenderRange; }
	...
	[SerializeField] private float bulletRenderRange;
	...
	protected Vector3 GetCrosshairHit() {

		Vector2 screenPoint = new Vector2(PlayerCamera.Instance.Camera.pixelWidth / 2.0f, PlayerCamera.Instance.Camera.pixelHeight / 2.0f);

		//Ray cameraRay = PlayerCamera.Instance.Camera.ScreenPointToRay(new Vector2(Screen.width / 2, Screen.height / 2));
		Ray cameraRay = PlayerCamera.Instance.Camera.ScreenPointToRay(screenPoint);

		//Debug.DrawRay(PlayerCamera.Instance.transform.position, cameraRay.direction, Color.green, 10f);
	...
	protected RaycastHit MuzzleCast(Vector3 direction) {
		
		//Debug.DrawLine(Muzzle.position, Muzzle.position + direction * 10.0sf, Color.red, 10.0f);
		Physics.Raycast(Muzzle.position, direction, out RaycastHit hit, float.MaxValue, BulletHitMask);
		return hit;
	}
	...
	protected Vector3 AddSpread(Vector3 direction) {
	...
	//Joakim Linna - joakimlinna.se
	protected void RenderBullet(Weapon weapon, Vector3 end)
    {
		if (weapon is AutoRifle)
		{
			if (Random.Range(0, 10) < 2)
			{
				float minOffset = Random.Range(.1f, 0.5f);
				float maxOffset = Random.Range(0.6f, .9f);

				Vector3 minPoint = Muzzle.position + ((end - Muzzle.position) * minOffset);
				Vector3 maxPoint = Muzzle.position + ((end - Muzzle.position) * maxOffset);

                PlayerWeapon.Instance.bulletRenderer.SetPositions(new Vector3[] { minPoint, maxPoint});
				ToggleBulletLine(true);
			}
			else
            {
				ToggleBulletLine(false);
			}
		}
	}
    ...
    {
		float intensity = Random.Range(2, 8);

		ToggleMuzzleFlash(true);

		Vector3 flashRotation = new Vector3(Random.Range(90f, 90f + 180f), -90, -90);

		PlayerWeapon.Instance.muzzleLight.transform.GetChild(0).transform.localRotation = Quaternion.Euler(flashRotation);

		PlayerWeapon.Instance.muzzleLight.intensity = intensity;
    }

	public void ToggleMuzzleFlash(bool enabled)
    {
		PlayerWeapon.Instance.muzzleLight.enabled = enabled;
		PlayerWeapon.Instance.muzzleLight.gameObject.SetActive(enabled);
	}

	public void ToggleBulletLine(bool enabled)
    {
		PlayerWeapon.Instance.bulletRenderer.enabled = enabled;
	}
}