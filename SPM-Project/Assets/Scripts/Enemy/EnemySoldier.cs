...
public class EnemySoldier : MobileEnemy, ILootable {
	...
	[SerializeField] [Tooltip("What radius the Soldier should use for avoidance while searching.")] private float searchingAvoidanceRadius;
	...
	private IEnumerator Search() {
		...
		StartCoroutine(AvoidanceLerp(defaultAgentAvoidanceRadius));
	...
	public override void StartAttackBehaviour() {
		agent.ResetPath();
		StartCoroutine(AvoidanceLerp(attackingAgentAvoidanceRadius));
		StartCoroutine("Attack");
	}
	...
	public override void StartSearchBehaviour() {
		agent.ResetPath();
		StartCoroutine(AvoidanceLerp(searchingAvoidanceRadius));
